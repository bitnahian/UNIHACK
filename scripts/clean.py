...
df_speed = df_speed[['LOCATION_CODE', 'LOCATION_DETAILS', 'CAMERA_TYPE','SCHOOL_ZONE_IND', 'SPEED_IND']]
df_red = df_red[['LOCATION_CODE', 'LOCATION_DETAILS', 'CAMERA_TYPE','SCHOOL_ZONE_IND', 'RED_LIGHT_CAMERA_IND']]
...
df_speed_counts = df_speed_counts.to_frame()
df_speed_counts.reset_index(level=0,inplace=True)

df_red_counts = df_red_counts.to_frame()
df_red_counts.reset_index(level=0,inplace=True)
...
df_speed_final.to_csv('../data/speed_data.csv',index=False)
df_red_final.to_csv('../data/red_light_data.csv',index=False)
